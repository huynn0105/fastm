...
class PersonalSettingCubit extends Cubit<PersonalSettingState> {
  ...
  Future<void> fetchData({bool showLoading = true}) async {
    ...
    final delMFastLeadResult = results[2] as BaseModel<DelMFastLeadModel>;
    ...
    if (personalSettingresult.status) {
      ...
      emit(state.copyWith(
        ...
        delMFastLead: delMFastLeadResult.data,
      ));
    } else {
      emit(state.copyWith(
  ...
  void clearData() {
    emit(const PersonalSettingState());
  }
}
