...
Future<Map<String, dynamic>> createBrochureMultiProducts({
  String title = "",
  List<MTradeBrochureProductModel> selectedProducts = const [],
  bool? showSaleInfo,
}) async {
  int pageTotal = 0;
  ...
  // if (selectedCategories?.valueAt(0)?.id != null && selectedProducts.first.groupIcon != null) {
  //   categoryIcon = await networkImage(selectedProducts.first.groupIcon!);
  // }

  for (var i = 0; i < selectedProducts.length; i++) {
    if (selectedProducts[i].medium?.isNotEmpty == true) {
      selectedProducts[i].productImage = await networkImage(selectedProducts[i].medium!.first);
      selectedProducts[i].brandLogoImage = await networkImage(selectedProducts[i].brandLogo ?? '');
    }
  }

  List<MTradeBrochureProductModel> topItems =
      selectedProducts.length >= 3 ? selectedProducts.sublist(0, 3) : selectedProducts;

  List<MTradeBrochureProductModel> listFirst = [];
  if (selectedProducts.length > 3) {
    listFirst = selectedProducts.length >= 9
        ? selectedProducts.sublist(3, 9)
        : selectedProducts.sublist(3, selectedProducts.length);
  }
  List<List<MTradeBrochureProductModel>> otherItems = [];
  if (selectedProducts.length > 9) {
    List<MTradeBrochureProductModel> otherSelected = selectedProducts.sublist(9);
    int chunkSize = 12;
