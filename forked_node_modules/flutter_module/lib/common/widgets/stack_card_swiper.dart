...
class AppStackCardSwiper extends StatefulWidget {
  /// list of widgets for the swiper
  final List<Widget> cards;

  /// controller to trigger unswipe action
  final AppSwiperController? controller;
  ...
  /// function that gets called with the boolean true when the last card gets unswiped and with the boolean false when there is no card to unswipe
  final Function(int index, AppStackCardSwiperDirection direction) onSwiping;

  /// direction in which the card gets swiped when triggered by controller, default set to right
  final AppStackCardSwiperDirection direction;

  ///
  final bool forceGoBackInLeftSwipe;

  ///
  final bool forceGoBackInRightSwipe;

  ///
  final Function(int index, AppStackCardSwiperDirection direction)? onPanEnd;

  const AppStackCardSwiper({
    Key? key,
    required this.cards,
    this.controller,
    ...
    this.onSwiped = emptyFunctionIndex,
    this.onEnd = emptyFunction,
    this.unswipe = emptyFunctionBool,
    this.onSwiping = emptyFunctionIndex,
    this.direction = AppStackCardSwiperDirection.right,
    this.forceGoBackInLeftSwipe = false,
    this.forceGoBackInRightSwipe = false,
    this.onPanEnd,
  })  : assert(maxAngle >= 0 && maxAngle <= 360),
        assert(threshold >= 1 && threshold <= 100),
        assert(direction != AppStackCardSwiperDirection.none),
        super(key: key);

  @override
  State createState() => _AppStackCardSwiperState();
}

class _AppStackCardSwiperState extends State<AppStackCardSwiper> with SingleTickerProviderStateMixin {
  double _left = 0;
  double _top = 0;
  double _total = 0;
  double _angle = 0;
  ...
  AppUnswipeCard? _lastCard;

  // ignore: prefer_final_fields
  List<AppUnswipeCard?> _lastCards = [];
  AppStackCardSwiperDirection detectedDirection = AppStackCardSwiperDirection.none;
  AppStackCardSwiperDirection detectingDirection = AppStackCardSwiperDirection.none;
  ...
  void initState() {
    ...
    if (widget.controller != null) {
      widget.controller!
        ...
        ..addListener(() {
          if (widget.controller!.state == AppStackCardSwiperState.swipe) {
            if (widget.cards.isNotEmpty) {
              switch (widget.direction) {
                case AppStackCardSwiperDirection.right:
                  _swipeHorizontal(context);
                  break;
                case AppStackCardSwiperDirection.left:
                  _swipeHorizontal(context);
                  break;
                case AppStackCardSwiperDirection.top:
                  _swipeVertical(context);
                  break;
                case AppStackCardSwiperDirection.bottom:
                  _swipeVertical(context);
                  break;
                case AppStackCardSwiperDirection.none:
                  break;
              }
        ...
        ..addListener(() {
          if (widget.controller!.state == AppStackCardSwiperState.swipeLeft) {
            if (widget.cards.isNotEmpty) {
              _left = -1;
              _swipeHorizontal(context);
              _animationController.forward();
        ...
        ..addListener(() {
          if (widget.controller!.state == AppStackCardSwiperState.swipeRight) {
            if (widget.cards.isNotEmpty) {
        ...
        ..addListener(() {
          if (widget.controller!.state == AppStackCardSwiperState.unswipe) {
      ...
      if (status == AnimationStatus.completed) {
        setState(() {
          if (_swipeTyp == 1) {
            ...
            widget.onSwiped(widget.cards.length, detectedDirection);
      ...
      child: GestureDetector(
        ...
        onPanUpdate: (tapInfo) {
          if (!widget.isDisabled) {
            ...
            if ((_left < -(widget.threshold) || _left > (widget.threshold)) && widget.horizontalSwipeEnabled) {
              _onHorizontalSwiping(context, index);
            }
          }
        },
        onPanEnd: (tapInfo) {
          if (!widget.isDisabled) {
            widget.onPanEnd?.call(index, AppStackCardSwiperDirection.none);
            _tapOnTop = false;
            _onEndAnimation();
            _animationController.forward();
          }
  ...
  void _calculateScale() {
    if (_scale <= 1.0 && _scale >= 0.9) {
      _scale = (_total > 0) ? 0.9 + (_total / 5000) : 0.9 + -1 * (_total / 5000);
    }
  }
  ...
  void _onEndAnimation() {
    if ((_left < -widget.threshold || _left > widget.threshold) && widget.horizontalSwipeEnabled) {
      _swipeHorizontal(context);
    } else if ((_top < -widget.threshold || _top > widget.threshold) && widget.verticalSwipeEnabled) {
      _swipeVertical(context);
    } else {
      _goBack(context);
    }
  ...
  void _swipeHorizontal(BuildContext context) {
    if (_left > widget.threshold || _left == 0 && widget.direction == AppStackCardSwiperDirection.right) {
      _swipedDirectionHorizontal = 1;
      detectedDirection = AppStackCardSwiperDirection.right;
    } else {
      _swipedDirectionHorizontal = -1;
      detectedDirection = AppStackCardSwiperDirection.left;
    }
    if (widget.forceGoBackInLeftSwipe && detectedDirection == AppStackCardSwiperDirection.left) {
      _goBack(context);
      return;
    }
    if (widget.forceGoBackInRightSwipe && detectedDirection == AppStackCardSwiperDirection.right) {
      _goBack(context);
      return;
    }
    setState(() {
      ...
      _leftAnimation = Tween<double>(
        begin: _left,
            ...
            ? (widget.direction == AppStackCardSwiperDirection.right)
  ...
  void _swipeVertical(BuildContext context) {
    setState(() {
      ...
      _topAnimation = Tween<double>(
        begin: _top,
            ...
            ? (widget.direction == AppStackCardSwiperDirection.bottom)
    ...
    if (_top > widget.threshold || _top == 0 && widget.direction == AppStackCardSwiperDirection.bottom) {
      _swipedDirectionVertical = -1;
      detectedDirection = AppStackCardSwiperDirection.bottom;
    } else {
      _swipedDirectionVertical = 1;
      detectedDirection = AppStackCardSwiperDirection.top;
    }
    (_left >= 0) ? _swipedDirectionHorizontal = 1 : _swipedDirectionHorizontal = -1;
    _vertical = true;
  }
  ...
  void _unswipe(AppUnswipeCard card) {
    _isUnswiping = true;
  ...
  void _onHorizontalSwiping(BuildContext context, int index) {
    if (_left > widget.threshold || _left == 0 && widget.direction == AppStackCardSwiperDirection.right) {
      detectingDirection = AppStackCardSwiperDirection.right;
    } else {
      detectingDirection = AppStackCardSwiperDirection.left;
    }
    widget.onSwiping(index, detectingDirection);
  }
}
...
void emptyFunctionIndex(int index, AppStackCardSwiperDirection direction) {}

void emptyFunctionBool(bool unswiped) {}

//to call the swipe or unswipe function from outside of the app swiper
class AppSwiperController extends ChangeNotifier {
  AppStackCardSwiperState? state;

  //swipe the card by changing the status of the controller
  void swipe() {
    state = AppStackCardSwiperState.swipe;
    notifyListeners();
  }

  //swipe the card to the left side by changing the status of the controller
  void swipeLeft() {
    state = AppStackCardSwiperState.swipeLeft;
    notifyListeners();
  }

  //swipe the card to the right side by changing the status of the controller
  void swipeRight() {
    state = AppStackCardSwiperState.swipeRight;
    notifyListeners();
  }

  //calls unswipe the card by changing the status of the controller
  void unswipe() {
    state = AppStackCardSwiperState.unswipe;
    notifyListeners();
  }
}

class AppUnswipeCard {
  Widget widget;
  bool horizontal;
  bool vertical;
...
enum AppStackCardSwiperState { swipe, swipeLeft, swipeRight, unswipe }

enum AppStackCardSwiperDirection { none, left, right, top, bottom }
