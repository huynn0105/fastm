...
class TextUtils {
  ...
  static String convertPrivateUrl(
    ...
    final defaultParams = AppData.instance.defaultParam.toJson().map((key, value) => MapEntry(key, value.toString()));
    final queryParameters = {
      ...uri.queryParameters,
      ...params,
      ...defaultParams,
  ...
  bool isNeedAccessToken(String url) {
    if (url.isEmpty) {
      return false;
    }

    List<String> allowedPrefixes = [
      EnvData.instance.baseUrl,
      'https://www.mfast.vn',
      'https://appay-rc.cloudcms.vn',
      'https://appay.cloudcms.vn',
      'http://support-rc.appay.vn',
    ];
  ...
  static String standardizePhoneNumber(String phoneNumber, {int maxDigits = 9}) {
    // Remove non-digits
    String standardPhoneNumber = phoneNumber.replaceAll(RegExp(r'\D'), '');

    // Prepend zero if phone number too short
    while (standardPhoneNumber.length < maxDigits) {
      standardPhoneNumber = '0$standardPhoneNumber';
    }

    // Trim
    final index = standardPhoneNumber.length - maxDigits;
    return standardPhoneNumber.substring(index);
  }
}
