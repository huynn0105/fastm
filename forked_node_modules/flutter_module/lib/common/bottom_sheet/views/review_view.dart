...
import 'package:flutter_module/features/mtrade/cubit/review/mtrade_review_cubit.dart';
...
class ReviewView extends StatelessWidget {
  ...
  final String appID;
  final MTradeSelectUserReviewModel? user;
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => MTradeReviewCubit(),
      child: BlocConsumer<MTradeReviewCubit, MTradeReviewState>(
        listener: (context, state) {
          if (state.reviewStatus.isFailure) {
            ToastProvider.instance.show(context: context, message: "Đánh giá thất bại");
          }
          if (state.reviewStatus.isSuccess) {
            ToastProvider.instance.show(context: context, message: "Đánh giá thành công");
        ...
        builder: (context, state) {
          if (state.status.isInitial) {
            WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
              if (user == null) {
                _onFetchListSelectUser(context);
              } else {
                _onFetchStatusReview(context);
              }
            });
          }
              ...
              children: [
                if (user == null) ...{
                  ...
                  UserReviewComponent(
                    ...
                    listStatus: state.statusReview?.getListStatus(user?.roleAlias ?? '') ?? [],
                    onSubmitReview: (rating, note) {
                      _updatePayload(
                        context,
                        rate: rating,
                        note: note,
                      ...
                      _onReviewUser(context);
                    },
                  ),
                },
                if (state.status.isLoading || state.reviewStatus.isLoading) ...{
                  const SizedBox(
                    width: double.infinity,
                    height: 110,
  ...
  _onFetchListSelectUser(BuildContext context) {
    final cubit = context.read<MTradeReviewCubit>();
    cubit.fetchListSelectUser(appID: appID);
  }

  _onFetchStatusReview(BuildContext context) {
    final cubit = context.read<MTradeReviewCubit>();
    cubit.fetchStatusReview();
  }

  _updatePayload(BuildContext context, {int? rate, String? note, String? userID, String? subsID}) {
    final cubit = context.read<MTradeReviewCubit>();
    cubit.updatePayload(
      appID: appID,
      rate: rate,
  ...
  _onReviewUser(BuildContext context) {
    final cubit = context.read<MTradeReviewCubit>();
    cubit.reviewUser();
  }
}
...
class SelectUserComponent extends StatelessWidget {
  ...
  final Function(MTradeSelectUserReviewModel)? onSelected;
  final List<MTradeSelectUserReviewModel> data;
