import 'package:flutter_module/common/enum/mfast/main_tab_bar_page_type.dart';
import 'package:flutter_module/common/enum/mfast/notification_category.dart';

part 'event/shipper.dart';
part 'event/mtrade.dart';
part 'event/mfast.dart';
part 'event/main_tab_bar.dart';
part 'event/web_view.dart';
part 'event/notification.dart';
part 'event/personal_setting.dart';
part 'event/client.dart';
part 'event/time_checking.dart';
