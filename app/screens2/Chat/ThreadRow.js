...
class ThreadRow extends PureComponent {
    ...
    return (
      <View style={styles.photoContainer}>
        ...
        {totalUnReadMessages <= 0 ? null : (
          <AppText style={isNotificationOn ? styles.badge : styles.badgeBlue}>
            {`${totalUnReadMessages}`}
          </AppText>
        )}
      </View>
  ...
  renderContent() {
    ...
    const lastMessageString = lastMessage ? lastMessage.getDisplayTextForLastMessage() : '...';
    const isUnread = totalUnReadMessages > 0;
    const titleStyle = isUnread ? styles.titleBoldText : styles.titleText;
    const titleSystemStyle = isUnread ? styles.titleBoldTextSystem : styles.titleTextSystem;
    const lastMessageStyle = isUnread ? styles.lastMessageBoldText : styles.lastMessageText;
    return (
      <View style={styles.contentContainer}>
        <AppText style={isSystem ? titleStyle : titleStyle} numberOfLines={TITLE_MAX_LINES}>
          {`${thread.titleString()}`}
        </AppText>
        {typingUsers.length > 0 ? (
          ...
          >
            <AppText style={lastMessageStyle} numberOfLines={LAST_MESSAGE_MAX_LINES}>
              {`${lastMessageString}`}
            </AppText>
          </View>
        )}
  ...
  renderExtraInfo() {
    ...
    return (
      <View style={styles.extraInfoContainer}>
        <AppText style={styles.timeText}>{lastUpdated}</AppText>
