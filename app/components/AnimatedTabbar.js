...
class AnimatedTabbar extends Component {
  ...
  movingSelectedView(left, tab) {
    const scaleX = 1 + ((Math.abs(this.state.selectedIndex - tab)) / 3);
    ...
    return (
      <View style={[styles.container, style]}>
        <View style={styles.buttonContainer}>
          {
            dataButtons.map((data) => this.renderButton(data.title, data.tag, data.tag === this.state.selectedIndex, data.unread))
          }
        </View>
        <Animated.View
