...
export default class Pie extends PureComponent {
  ...
  render() {
    ...
    const _innerRadius = this._calculateRadius(innerRadius || '50%', maxRadius, 0);
    const _padAngle = typeof padAngle !== 'undefined' ? padAngle : 0.05;
    const _totalValue = data.reduce(
      (previousValue, item) => (previousValue += valueAccessor({ item })),
      0,
    );

    const arcs = data.map((item, index) => {
      const _percentage = (valueAccessor({ item }) / _totalValue) * 100;
      const _disabledEnhancedPadAngle = _percentage < 5; // smaller 5%
      const _enhancedOuterRadius = indexToFocus === index ? _outerRadius + elevation : _outerRadius;
      const _enhancedPadAngle =
        indexToFocus === index && !_disabledEnhancedPadAngle
          ? _padAngle + elevation / 100
          : _padAngle;
      const arc = shape
        .arc()
        .outerRadius(_enhancedOuterRadius)
        .innerRadius(_innerRadius)
        .padAngle(_enhancedPadAngle); // Angle between sections
    ...
    const pieSlices = shape
      ...
      .sort(sort)
      .startAngle(animate ? startAngle : 0)
