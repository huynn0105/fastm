...
class HeaderApp extends React.PureComponent {
  ...
  _renderTitle(props, options) {
    ...
    let style = [
      ...
      getTitleOffsets(layoutPreset, false, options.hasLeftComponent, options.hasRightComponent),
  ...
  _renderModularSubView(props, name, renderer, labelStyleInterpolator, buttonStyleInterpolator) {
    ...
    const ButtonContainer = ({ children }) => (
      <Animated.View style={buttonStyleInterpolator({ ...this.props, ...props })}>
        {children}
      </Animated.View>
    );

    const LabelContainer = ({ children }) => (
      <Animated.View style={labelStyleInterpolator({ ...this.props, ...props })}>
        {children}
      </Animated.View>
    );
  ...
  render() {
    ...
    const statusBarHeight = this.props?.insets?.top || 0;
    ...
    const containerStyles = [
      ...
      { height: appBarHeight + statusBarHeight, paddingTop: statusBarHeight },
      safeHeaderStyle,
    ];
    ...
    return (
      ...
      >
        <View forceInset={forceInset} style={containerStyles}>
          {background}
...
export default withOrientation(withSafeAreaInsets(HeaderApp));
