...
import { StyleSheet, View, FlatList, Dimensions, Text, Image } from 'react-native';

import { removeNotificationFromPendingList } from '../../redux/actions';
...
class NotiInApp extends PureComponent {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (
      pendingNotificationList !== nextProps.pendingNotificationList &&
      nextProps.pendingNotificationList &&
      !isChange
    ) {
      this.setState({
  ...
  renderSystemNoti = (notification, index) => {
    ...
    return (
      <NotiRow
        image={
          notification.category === NOTIFICATION_CATEGORIES.ADMIN ? ICON_PATH.anna : ICON_PATH.elly
        }
        name={
          notification.category === NOTIFICATION_CATEGORIES.ADMIN
            ? 'Anna - Trợ lý tin tức'
            : 'Elly - Trợ lý riêng của bạn'
        }
  ...
  renderChatNoti = (notification, index) => {
    const { isExpend } = this.state;
    const number = this.pendingGiftedNotificationList()?.length - (index || 0 + 1);
    const parseNoti = notification.extraData;
    return (
      <NotiRow
        image={parseNoti.image}
        name={parseNoti.title}
        title={parseNoti.title}
        content={parseNoti.details}
        index={index}
        notification={notification}
  ...
  renderStackColpand = () => {
    const pendingNotificationList = this.props.pendingNotificationList;
    if (pendingNotificationList?.length <= 0) return <View />;
    return this.renderNotiRow({ item: pendingNotificationList?.[0], index: 0 });
  };
  ...
  render() {
    ...
    return (
      <View style={styles.container} pointerEvents={'box-none'}>
        <View style={{ flex: 0, width: SCREEN_SIZE.width }}>
          <FlatList
...
const mapDispatchToProps = (dispatch) => ({
  removeNotificationFromPendingList: (notification) =>
    dispatch(removeNotificationFromPendingList(notification)),
});
