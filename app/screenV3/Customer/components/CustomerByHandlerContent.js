...
import { getStatisticCustomerByHandler } from '../../../redux/actions/actionsV3/customerAction';
import { STATUS_ITEM } from '../Customer.constants';

const ARRAY_COLOR = ['#005fff', '#dc41d7', '#ff499c', '#ff8469', '#ffc252'];
const CustomerByHandlerContent = memo((props) => {
  ...
  const onGetData = useCallback(() => {
    setIsLoading(true);
    dispatch(
      getStatisticCustomerByHandler((isSuccess, result) => {
        if (isSuccess) {
          setData(result);
        }
        setIsLoading(false);
  ...
  return (
    <>
      <HeaderSection title={`Khách hàng theo nghiệp vụ bán hàng`} />
...
export default CustomerByHandlerContent;
