...
import { LIST_TYPE_CUSTOMER, TAB_TYPE } from '../Customer.constants';
import { getStatisticCustomerByProgress } from '../../../redux/actions/actionsV3/customerAction';

const ARRAY_COLOR = ['#005fff', '#dc41d7', '#ff499c', '#ff8469', '#ffc252'];
const CustomerByStatusContent = memo((props) => {
  ...
  const [type, setType] = useState(TAB_TYPE.LOAN);
  ...
  const onPressName = useCallback(
    ({ filters }) => {
      ...
      onChangeFilterFromStatistic?.(filterParse, type);
    },
    [onChangeFilterFromStatistic, type],
  );
  ...
  const onGetData = useCallback(() => {
    ...
    dispatch(
      getStatisticCustomerByProgress(payload, (isSuccess, result) => {
        setIsLoading(false);
        if (isSuccess) {
          setData(result);
        }
  ...
  return (
    <>
      <HeaderSection title={`Khách hàng theo trạng thái`} />
      <View>
        <ListButtonFilter
          data={LIST_TYPE_CUSTOMER.data?.slice(2)}
          disabled={isLoading}
      ...
      <View style={styles.cardContainer}>
        <View style={styles.chartInfoContainer}>
          ...
          <View style={styles.infoContainer}>
            <AppText style={styles.infoTitle}>{'Tổng khách hàng'}</AppText>
...
export default CustomerByStatusContent;
